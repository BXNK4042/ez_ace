...
export function DocumentUpload({ classes }: { classes: { id: string; name: string }[] }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); const form = event.currentTarget; const file = fileInput.current?.files?.[0]; if (!file) return;
    ...
    setBusy(true); setStatus("Uploading…"); const data = new FormData(form); const classId = String(data.get("classId"));
    try {
      ...
      setStatus("Uploaded"); form.reset(); router.refresh();
